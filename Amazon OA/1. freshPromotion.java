...
class Main {
    private static int freshPromotion(String[][] codeList, String[] shopCart) {
        int m = codeList.length;
        int shopLen = shopCart.length;
        // two pointers for each array
        int codeIdx = 0, shopIdx = 0;
        // traverse the two array
        while (shopIdx < shopLen && codeIdx < m) {
            String cur = shopCart[shopIdx];
            // if the first fruit is anything of codeList or it matches the cur fruit
            if ((codeList[codeIdx][0] == "anything" || codeList[codeIdx][0].equals(cur)) && rightOrder(shopCart, shopIdx, codeList[codeIdx])) {
                shopIdx += codeList[codeIdx++].length;
            } else {
                shopIdx++;
            }
        }
